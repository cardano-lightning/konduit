...
impl Backing {
    ...
    pub fn snapshot(
        ...
        self.append(utxos.into_iter().map(Chain::new).collect::<Vec<_>>());
        todo!()
    }
    ...
    /// + If the parent already has child, then the existing lineage is split off into a
    ...
    /// + Else the child is succeeding not the parent, then something has gone very wrong.
    ...
    /// The live chain tip with the highest locked amount, if any live chain exists.
    /// Used to determine current spendable capacity and next cheque index.
    pub fn best_live(&self) -> Option<&BackingUtxo> {
        self.0
            .iter()
            .filter(|c| c.is_live())
            .map(|c| c.last())
