...
export async function endpointUtxosAt(ctx) {
  try {
    ...
    return ctx.json(await Promise.all(utxos.map(async (utxo) => {
      let reference_script = null;
      let reference_script_version = null;

      if (utxo.reference_script_hash) {
        const { cbor } = await ctx.blockfrost(`/scripts/${utxo.reference_script_hash}/cbor`);
        reference_script = cbor;
        reference_script_version = guessPlutusVersion(utxo.reference_script_hash, cbor);
      }
...
function guessPlutusVersion(scriptHash, script) {
  const scriptHashV = (v) => blake2b(28).update(Buffer.concat([
    Buffer.from([v]),
