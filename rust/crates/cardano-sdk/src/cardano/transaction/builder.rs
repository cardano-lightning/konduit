...
impl Transaction<state::InConstruction> {
    ...
    ///
    /// ```rust
    /// # use cardano_sdk::{ChangeStrategy, ProtocolParameters, Transaction, input, output, value, address};
    ...
    ///
    /// ```rust
    /// # use cardano_sdk::{ChangeStrategy, PlutusData, PlutusVersion, ProtocolParameters, SlotBound, Transaction};
    /// # use cardano_sdk::{address, assets, input, output, plutus_script, value};
    /// # use std::collections::btree_map::BTreeMap;
    /// # use indoc::indoc;
    /// let resolved_inputs = BTreeMap::from([
    ///   (
