...
#[cfg(feature = "wasm")]
pub mod wasm {
    use crate::types::wasm::TransactionSummary;
    use cardano_connector::CardanoConnector;
    use cardano_sdk::{
        wasm::{self, Credential, NetworkId, VerificationKey, WasmProxy},
        wasm_proxy,
    };
    ...
    wasm_proxy! {
        #[doc = "A Cardano connector interface that's wasm-friendly."]
        Connector => super::Connector<HttpClient>
    }

    #[wasm_bindgen]
    impl Connector {
        #[wasm_bindgen(js_name = "new")]
        pub async fn _wasm_new(base_url: &str) -> wasm::Result<Self> {
            Ok(Self(
                super::Connector::new(HttpClient::new(base_url)).await?,
            ))
        }

        #[wasm_bindgen(getter, js_name = "networkId")]
        pub fn _wasm_network_id(&self) -> NetworkId {
            <NetworkId as WasmProxy>::OriginalType::from(self.network).into()
        }
        ...
        pub async fn _wasm_balance(&self, verification_key: &VerificationKey) -> wasm::Result<u64> {
            Ok(self.balance((*verification_key).into()).await?)
        }
        ...
        pub async fn _wasm_transactions(
            ...
            Ok(self
                .transactions(&credential.clone().into())
                .await?
                .into_iter()
                .map(From::from)
                .collect())
