...
pub(crate) async fn execute(connector: impl CardanoConnect, args: Args) -> anyhow::Result<()> {
    // TODO: That's probably going to be very common to all commands; we might want to move one
    // level up and receive this as argument instead.
    let wallet = args
        .wallet
        .as_shelley()
        .ok_or(anyhow!("expected a Shelley address as wallet"))?;
