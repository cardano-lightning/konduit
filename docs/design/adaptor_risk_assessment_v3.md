...
consensus. While immutable finality ($k=2160$ blocks, $\approx$ 36 hours) offers
...
but the secret is not revealed by the recipient for a prolonged period. While
~99.7% of payments resolve within seconds, "stuck" payments can persist for
minutes or even hours.

This creates a structural "vulnerability window" where the Adaptor's ada is
locked on cardano, but the corresponding bitcoin value is in limbo. The primary
causes are:
...
- **Agency-driven (griefing):** An adversarial recipient (or intermediary)
  intentionally holds the secret until just before the HTLC timeout. This is a
  known "griefing attack" intended to lock liquidity or exploit price movement.

**Exploitation potential.** High. Because the bitcoin secret is controlled by
the recipient, they can perfectly time the revelation to coincide with market
volatility. This allows them to effectively "look back" at price changes before
deciding to finalize the trade at the Adaptor's expense.

**Impact surface.** Operational & financial. Results in an extended
vulnerability window for market volatility and exchange rate drift. While the
principal is not technically "lost" (it either settles or refunds), the Adaptor
is forced to hold a stale position, potentially leading to margin erosion.
...
- **Agency-driven (jamming):** An attacker intentionally initiates payments to
  themselves through Adaptor's channels, deliberately withholding the secret to
  "slow-jam" slots. Each lightning channel has a structural limit of 483
  concurrent HTLC slots; an attacker can render a channel unusable by filling
  these slots with low-value, stuck payments.
...
with many small amounts (dust-limit).

**Exploitation potential.** High. Lightning channels have a fixed, public limit
of 483 slots. An attacker can observe Adaptor's node capacity and precisely
calculate the resources needed to "jam" the service into inoperability.
...
As shown in [Appendix A](#appendix-a), there is a non-negligible statistical
...
**Analysis.** This risk involves an unauthorized actor gaining access to the
signing keys used by Adaptor to sign Cardano transactions and lightning channel
updates. Because the Konduit protocol is trust-minimized, the security of the
funds is ultimately anchored in the possession of these keys.

A compromise can occur through several vectors:

- **Physical/Remote Access:** Unauthorized entry into the server environment or
  a breach of the host operating system.
- **Misconfiguration:** Improperly set permissions (e.g., world-readable files)
  or secrets exposed in environment logs/backups.
- **Supply Chain:** Vulnerabilities in the node software dependencies that allow
  for "key exfiltration" via a malicious update.

**Technical competency.** Moderate to high. While basic misconfigurations can be
exploited by low-skill actors, a targeted exfiltration from a hardened
production environment typically requires sophisticated knowledge of linux
security and potentially cardano/bitcoin signing libraries.

**Resource commitment.** Variable. Costs range from negligible (exploiting an
open port) to significant (targeted social engineering or long-term zero-day
research).

**Exploitation potential.** High (post-access). Once the key is accessed,
exploitation is trivial and immediate. The attacker simply signs a "drain"
transaction to move all ada and bitcoin to an attacker-controlled address. There
is no "undo" mechanism for a signed blockchain transaction.

**Impact surface.** Critical. Results in the total loss of all funds currently
owed to the Adaptor, as well as any static collateral or liquidity currently
held in the compromised wallet.
...
### Appendix A: Statistical Analysis of Temporal Divergence {#appendix-a}

FIXME :: this needs to be tidied.
