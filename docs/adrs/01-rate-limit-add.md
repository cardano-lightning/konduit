...
> [!NOTE] Temporal divergence is a divergence in time between Cardano's Plutus
> (posix time) vs BLN's commitments that use bitcoin block height. Bitcoin
> blocks are produced via a process that can be modelled as an exponential
> distributions. Divergence can occur simply because this is a random process,
> although its more likely to be significant due to external factors such as a
> sudden drop in hashing power. Detailed calculations are elsewhere.
