...
use crate::types::{
    BulkPatternsBody, Checkpoint, Datum, Deleted, ForcedRollback, ForcedRollbackBody, Health,
    Match, MatchFilters, MatchStatus, Metadata, Pattern, PutPatternBody, Script, Strict,
};
use crate::{BasicAuth, Error, Result};
use reqwest::blocking::Client as HttpClient;
use reqwest::{Method, Url, header::ACCEPT};
...
impl Client {
    ...
    pub fn health(&self) -> Result<KupoResponse<Health>> {
    ...
    pub fn all_matches(&self, filters: &MatchFilters) -> Result<KupoResponse<Vec<Match>>> {
        let query = filters.to_query_items();
        self.get::<Vec<Match>>("/matches", &query)
    }
    ...
    pub fn matches(
        ...
        let query = filters.to_query_items();
        self.get::<Vec<Match>>(&path, &query)
    }

    /// Delete all matches matching the given pattern.
    ///
    /// Only allowed if the provided pattern is not currently active.
    pub fn delete_matches(&self, pattern: &Pattern) -> Result<KupoResponse<Deleted>> {
        let path = format!("/matches/{}", urlencoded(pattern));
        self.delete::<Deleted>(&path)
    }
    ...
    pub fn datum(&self, datum_hash: &crate::Blake2b256) -> Result<KupoResponse<Option<Datum>>> {
        let path = format!("/datums/{datum_hash}");
        self.get_nullable::<Datum>(&path, &[])
    }
    ...
    pub fn script(&self, script_hash: &crate::Blake2b224) -> Result<KupoResponse<Option<Script>>> {
        let path = format!("/scripts/{script_hash}");
        self.get_nullable::<Script>(&path, &[])
    }

    // ---------------------------------------------------------------- Patterns

    /// Retrieve all patterns currently configured on the server.
    pub fn patterns(&self) -> Result<KupoResponse<Vec<Pattern>>> {
        self.get::<Vec<Pattern>>("/patterns", &[])
    }
    ...
    pub fn put_pattern(
        ...
        self.put::<_, Vec<Pattern>>(&path, &body)
    }

    /// Remove a pattern from the database and active filtering.
    pub fn delete_pattern(&self, pattern: &Pattern) -> Result<KupoResponse<Deleted>> {
        let path = format!("/patterns/{}", urlencoded(pattern));
        self.delete::<Deleted>(&path)
    }
    ...
    pub fn put_patterns(
        ...
        self.put::<_, Vec<Pattern>>("/patterns", &body)
    }

    // ------------------------------------------------------------- Checkpoints

    /// Retrieve a sample of all checkpoints currently in the database.
    pub fn checkpoints(&self) -> Result<KupoResponse<Vec<Checkpoint>>> {
        self.get::<Vec<Checkpoint>>("/checkpoints", &[])
    }
    ...
    pub fn metadata(
        ...
        let query: Vec<(String, Option<String>)> = transaction_id
            ...
            .unwrap_or_default();
        self.get::<Vec<Metadata>>(&path, &query)
    }
    ...
    fn get_nullable<T: serde::de::DeserializeOwned>(
        &self,
        path: &str,
        query: &[(String, Option<String>)],
    ...
    fn delete<T: serde::de::DeserializeOwned>(&self, path: &str) -> Result<KupoResponse<T>> {
