...
pub struct Commitments(#[n(0)] BTreeMap<Lock, Tag>);

impl Commitments {
    pub fn get(&self, lock: &Lock) -> Option<Tag> {
        self.0.get(lock).cloned()
    }

    /// Commit `lock` to `tag`. Errors if `lock` is already committed
    pub fn try_set(&mut self, lock: Lock, tag: Tag) -> Result<(), Error> {
...
pub enum Error {
    ...
    AlreadyCommitted { lock: Lock, existing: Tag },
}
