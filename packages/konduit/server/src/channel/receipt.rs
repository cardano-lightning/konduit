...
use konduit_wire::auth::{squash::SquashProposal, state::Receipt as WireReceipt};
...
impl Receipt {
    ...
    fn unlockeds(&self) -> impl Iterator<Item = Unlocked<Verified>> {
        self.cheques().filter_map(Cheque::<Verified>::as_unlocked)
    }

    fn lockeds(&self) -> impl Iterator<Item = Locked<Verified>> {
